...
Bind 是 LDAP 中的**认证操作**，用于验证用户身份并建立连接的权限上下文。Bind 操作的核心作用是**验证身份**，LDAP 服务器在收到 Bind 请求后会在服务器端验证密码，客户端不需要（也无法）直接读取密码属性进行比较。

**Bind 操作的作用**：
1. **验证身份**：验证提供的 DN 和密码是否正确
2. **建立权限上下文**：Bind 成功后，连接以该身份运行，拥有相应的权限
3. **决定操作权限**：不同的身份有不同的权限（如搜索、修改、删除等）

**ACT-LDAP 认证中的两步 Bind**：

在 ACT-LDAP 认证流程中，需要执行两次 Bind 操作：

| 阶段 | Bind 身份 | 目的 |
|------|----------|------|
| **第一次 Bind** | 管理员账号 | 获取搜索权限，用于搜索用户 DN |
| **第二次 Bind** | 用户 DN | 验证用户密码是否正确 |
